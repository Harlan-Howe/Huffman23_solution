...
class PriorityQueue (Generic[T]):
    ...
    def is_a_heap(self) -> bool:
        ...
        while self.in_bounds(index):
            ...
            myNode = self.node_at_index(index)
            if self.has_left_child(index):
                leftNode = self.node_at_index(self.left_child_of_index(index))
                if self.a_has_priority_over_b(leftNode,myNode):
                    logging.info(f"Found a heap violation at left child of node at {index}.")
                    return False
            if self.has_right_child(index):
                rightNode = self.node_at_index(self.right_child_of_index(index))
                if self.a_has_priority_over_b(rightNode, myNode):
                    logging.info(f"Found a heap violation at right child of node at {index}.")
                    return False
            # ----------------------
            index += 1
    ...
    def heapify_up(self, index: int):
        ...
        if index == 0:
            return
        myNode = self.node_at_index(index)
        parentNode = self.node_at_index(self.parent_of_index(index))
        if self.a_has_priority_over_b(myNode,parentNode):
            self.set_node_at_index(self.set_node_at_index(myNode,self.parent_of_index(index)),index)
        self.heapify_up(self.parent_of_index(index))
        # ----------------------
    ...
    def heapify_down(self, index: int = 0):
        ...
        if self.in_bounds(left_index):
            ...
            # There are three cases: {right & left, left only, no children, and if we got here, it isn't the last one.}.
            left_node: "Node" = self.node_at_index(left_index)
            if self.has_right_child(index):
                right_node: "Node" = self.node_at_index(right_index)
                if self.a_has_priority_over_b(right_node, current_node):
                    if self.a_has_priority_over_b(right_node, left_node):
                        self.set_node_at_index(self.set_node_at_index(current_node, right_index), index)
                        self.heapify_down(right_index)
                        return
                    else:
                        self.set_node_at_index(self.set_node_at_index(current_node, left_index), index)
